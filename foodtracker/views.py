...
from .models import User, Food, FoodCategory, FoodLog, Image, Weight ,UserCalories,Coaches,Premium,Foodpref
...
def register(request):
    ...
    if request.method == 'POST':
        ...
        ##weight_log.save()
            ...
            userfoodpref = Foodpref(users.id,username,pref)
            try:
...
def coach_message(request,coach_id):
    ...
    if request.method == 'POST':
        ...
        data = {'Messages': [{
                        "From": {"Email": "avneeptestcase@gmail.com","Name": "Me"},
                        "To": [{"Email": str(coach.email),"Name": "You"}],
                        "Subject": "From "+str(user.username)+" to "+str(coach.name),
                        "HTMLPart": "<h1>"+str(sub)+",</h1><br>"+"<h2>"+str(text)+"</h2><br><h4>respond back to "+str(user.email)+" please"}]
                }
        result = mailjet.send.create(data=data)
        print(result.json())
...
def shop_list(request):
    ...
    Pref = Foodpref.objects.get(user=request.user)
    pref = int(Pref.pref)
    api_key = 'b132dbed8693f3e2f0f104718df0861b'
    api_secret = 'aef661c0059e1f0243da1eb14c9b8954'
    if(pref == 1 and flag==0):
        food = Food.objects.all().filter(category_id=2)
        print(food)
        s = ""
        for fd in food:
            s = s+fd.food_name+", "
        ...
        data = {'Messages': [{
                        "From": {"Email": "avneeptestcase@gmail.com","Name": "Me"},
                        "To": [{"Email": str(Email),"Name": "You"}],
                        "Subject": "Your shopping list for today",
                        "TextPart": "Greetings from Dietician!",
                        "HTMLPart": "<h3><input type='checkbox'> eggs<br><input type='checkbox'> chicken<br><input type='checkbox'> rice<br><input type='checkbox'> olive oil<br><input type='checkbox'> oats<br><input type='checkbox'> vegetables<br><input type='checkbox'> milk<br><input type='checkbox'> paneer<br><input type='checkbox'> bread<br></h3><br><br><b><p>food:-<br>"+str(s)+"</p></b>"}]
                }
        result = mailjet.send.create(data=data)
        print(result.json())
        flag=1
    if(pref == 2 and flag==0):
        food = Food.objects.all().filter(category_id=1)
        print(food)
        s = ""
        for fd in food:
            s = s+fd.food_name+", "
        ...
        data = {'Messages': [{
                        ...
                        "Subject": "Your shopping list for today",
                        "TextPart": "Greetings from Dietician!",
                        "HTMLPart": "<h3><input type='checkbox'> soybean<br><input type='checkbox'> tofu<br><input type='checkbox'> kidney beans<br><input type='checkbox'> olive oil<br><input type='checkbox'> oats<br><input type='checkbox'> vegetables<br><input type='checkbox'> milk<br><input type='checkbox'> paneer<br></h3><br><br><b><p>food:-<br>"+str(s)+"</p></b>"}]
                }
        result = mailjet.send.create(data=data)
        print(result.json())
        flag=1
    if(pref == 3 and flag==0):
        food = Food.objects.all().filter(category_id=3)
        print(food)
        s = ""
        for fd in food:
            s = s+fd+", "
        ...
        data = {'Messages': [{
                        ...
                        "Subject": "Your shopping list for today",
                        "TextPart": "Greetings from Dietician!",
                        "HTMLPart": "<h3><input type='checkbox'> soybean<br><input type='checkbox'> tofu<br><input type='checkbox'> plant protein bars<br><input type='checkbox'> olive oil<br><input type='checkbox'> oats<br><input type='checkbox'> vegetables<br><input type='checkbox'> kidney beans<br><input type='checkbox'> almond milk<br><input type='checkbox'> oat milk<br><input type='checkbox'> fruits<br></h3><br><br><b><p>food:-<br>"+str(s)+"</p></b>"}]
                }
        result = mailjet.send.create(data=data)
        print(result.json())
        flag=1
...
def live(request,coach_id):
    coach = Coaches.objects.get(id=coach_id)
    flag = 0
    if flag==0:
        cmd = "python3 linkgen.py "+str(coach.email)+" "+str(request.user.email)
        print(cmd)
        os.system(cmd)########################################################################
        flag=1
    return render(request,'live.html',{"name":coach.name})
